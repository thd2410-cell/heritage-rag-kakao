...
5. 최근 source에서 서로 다른 국가유산명을 최대 5개까지 뽑아 최근 주제 히스토리로 사용한다.
6. 사용자가 새 유산명을 명확히 말하면 그 유산으로 주제를 전환한다.
7. 새 유산명이 없고 질문이 후속 질문이면 현재 주제를 유지한다.
8. `아까`, `이전`, `전에`, `원래`, `다시`, `돌아` 같은 표현이 있으면 직전 주제만 보지 않고 이전 주제로 복귀할 수 있게 한다.
...
최근 주제 히스토리 조회
  예: ["흥인지문", "서울 숭례문", "경복궁"]
  ↓
질문 안에 명확한 새 국가유산명이 있는지 확인
  - 직접 이름 포함
  - alias 적용 후 이름 포함
  - fuzzy_name score가 높은 경우
  - 검색 점수가 충분히 높고 주제 전환 표현이 있는 경우
  ↓
새 유산명이 있으면 그 유산으로 전환
새 유산명이 없으면 현재 주제 유지
`아까/이전/다시/돌아` 표현이 있으면 이전 주제로 복귀
```
...
사용자: 흥인지문은?
새 유산명 감지 → 흥인지문으로 전환
현재 주제 히스토리: [흥인지문, 서울 숭례문]

사용자: 근처에 뭐 있어?
새 유산명 없음 → 흥인지문 유지
내부 검색 질문: 흥인지문에 대해 근처에 뭐 있어?

사용자: 아까 그거 화재 얘기 다시 해줘
복귀 표현 감지 → 이전 주제 서울 숭례문으로 복귀
내부 검색 질문: 서울 숭례문에 대해 아까 그거 화재 얘기 다시 해줘
```

주의:

- 사용자가 아무 단서 없이 “그건?”이라고만 하면 현재 주제를 유지한다.
- “아까 것”이 여러 개일 경우 현재는 가장 최근의 이전 주제로 복귀한다.
- 완전한 대화 메모리라기보다는 최근 source 기반의 lightweight topic stack이다.
